...
def k_states(X, n_clusters=8, max_iter=100, dissim=matching_dissim, init='Cao', n_init=1, verbose=0):
    ...
    for i in range(len(K_results)):
        if i+2>=len(K_results):
            break
        disc_difference = K_costs[i]-K_costs[i+1]
        if disc_difference<average_cost:
            best_K = i
            break

    return K_results[best_K], K_maps[best_K], K_labels[best_K], K_costs[best_K], K_n_iters[best_K]

def k_modes_continued(X, n_clusters, max_iter, dissim, init, n_init, verbose, premade_centroids):
    """k-modes algorithm"""
